<?php
/**
 * @version		$Id: captcha.php 253 2009-01-07 23:56:26Z louis $
...
class JXCaptcha extends JObject
{
	 ...
	 */
	static function &getInstance($type = 'image', $options = array())
	{
	 ...
	 *	$captchas = $app->getUserState('jxcaptcha.captcha');
	 *
	...
	function clean($session = true, $expired = true)
	{
		...
		{
			// Get the captchas
			$captchas = $app->getUserState($this->_namespace.'captcha');
			...
			if (is_array($captchas))
			{
				...
				{
					// Delete the file
					if (!$this->_deleteFile($this->filePath.DIRECTORY_SEPARATOR.$captcha['file']))
					{
						// Error: delete failed
						return false;
					}
			...
			// Clean the session
			$app->setUserState($this->_namespace.'captcha', null);
		}
		...
		if ($expired == true)
		{
			...
			foreach ($files as $file)
			{
				...
				{
					$mtime = filemtime($this->filePath.DIRECTORY_SEPARATOR.$file);
					//$ltime = $config->getValue('lifetime');
					$ltime = (int) $config->get('lifetime');
					$config->get('mailfrom');
					$etime = time() - ($ltime * 60);
					...
					{
						// Delete the file
						if (!$this->_deleteFile($this->filePath.DIRECTORY_SEPARATOR.$file))
						{
							// Error: delete failed
							return false;
						}
...
class JXCaptchaAudio extends JXCaptcha
{
	...
	function __construct($options = array())
	{
		...
		$defaults	= array('application'		=> '/usr/bin/flite',
							'charset'			=> '0123456789',
							'filePath'			=> JPATH_SITE.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'captcha'.DIRECTORY_SEPARATOR.'audio',
							'format'			=> 'wav',
							'phrases'			=> array(
														"The %i numbers are %s",
	...
	function create()
	{
		...
		if (is_null($app->getUserState($this->_namespace.'captcha')))
		{
			$app->setUserState($this->_namespace.'captcha', array($this->id => $return));
		}
		else
		{
			$captcha = $app->getUserState($this->_namespace.'captcha');
			$captcha[$this->id] = $return;

			$app->setUserState($this->_namespace.'captcha', $captcha);
		}
...
class JXCaptchaImage extends JXCaptcha
{
	...
	function __construct($options = array())
	{
		...
		$defaults	= array('charset'			=> 'ABCDEFGHIJKLMNOPQRSTUVWXYZ',
							'color'				=> true,
							'shadow'			=> false,
							'backgroundPath'	=> dirname( __FILE__ ).DIRECTORY_SEPARATOR.'backgrounds',
							'fontPath'			=> dirname( __FILE__ ).DIRECTORY_SEPARATOR.'fonts',
							'filePath'			=> JPATH_SITE.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'captcha'.DIRECTORY_SEPARATOR.'image',
							'backgrounds'		=> array(),
							'fonts'				=> array(),
							'minFont'			=> 11,
							'maxFont'			=> 14,
		...
		if (count($this->fonts = JFolder::files($this->fontPath, '.', false, true, array('.svn', 'CVS', 'index.html'))) < 1)
		{
		...
		if (($this->backgrounds = JFolder::files($this->backgroundPath, '.', false, true, array('.svn', 'CVS', 'index.html'))) === false)
		{
	 ...
	 * @return	bool	True on success/false on failure
	 */
	static function test($options = array())
	{
		// Verify GD's presence
		if (!function_exists('gd_info')) {
			JError::raiseNotice(500, JText::_('CAPTCHA_GD_LIBRARY_NOT_AVAILABLE'));
			return false;
		}
		...
		switch ((array_key_exists('format', $options)) ? $options['format'] : null)
		{
			case 'png':
				if (!function_exists('imagepng')) {
					JError::raiseNotice(500, JText::_('CAPTCHA_INVALID_IMAGE_FORMAT_PNG'));
					return false;
				}
				break;
			...
			case 'jpeg':
				if (!function_exists('imagejpeg')) {
					JError::raiseNotice(500, JText::_('CAPTCHA_INVALID_IMAGE_FORMAT_JPEG'));
					return false;
				}
				break;
			...
			case 'gif':
				if (!function_exists('imagegif')) {
					JError::raiseNotice(500, JText::_('CAPTCHA_INVALID_IMAGE_FORMAT_GIF'));
					return false;
				}
				break;
	...
	function create($id = null)
	{
		...
		if (is_null($app->getUserState($this->_namespace.'captcha')))
		{
			$app->setUserState($this->_namespace.'captcha', array($return['id'] => $return));
		}
		else
		{
			$captcha = $app->getUserState($this->_namespace.'captcha');
	 ...
	 * @return	bool	True on success/false on failure
	 */
	function validate($id, $input, $caseSensitive = false, $clean = true)
	{
		$app = JFactory::getApplication();
	...
	{
		$format		= $this->format;
		$filename	= $this->filePath.DIRECTORY_SEPARATOR.$this->filename;
