...
JM(document).ready(function(){
	...
	JM('.jm_login form#jm-login-form').submit(function(){  
        if(JM("#jm-input-username").val()=="") {
    	   	JM("#jm-login-error").html(Joomla.JText._('JM_REQUIRED_USERNAME')).show(); 
    		return false;
    	}
    	if(JM("#jm-input-password").val()==""){
    		JM("#jm-login-error").html(Joomla.JText._('JM_REQUIRED_PASSWORD')).show(); 
    		return false;
    	}
	...
	JM('.jm_login form#jm-form-register').submit(function(){ 
        if(JM("#jm-input-name").val()==""){
    		JM("#jm-registration-error").html(Joomla.JText._('JM_REQUIRED_NAME')).show();
    		JM("#jm-input-name").focus();
    		return false;
    	}
    	if(JM("#jm-input-user-name").val()==""){
    		JM("#jm-registration-error").html(Joomla.JText._('JM_REQUIRED_USERNAME')).show();
    		JM("#jm-input-user-name").focus();
    		return false;
    	}
    	if(JM("#jm-input-pass").val()==""){
    		JM("#jm-registration-error").html(Joomla.JText._('JM_REQUIRED_PASSWORD')).show();
    		JM("#jm-input-pass").focus();
    		return false;
    	}
    	if(JM("#jm-input-confirm-pass").val()==""){
    		JM("#jm-registration-error").html(Joomla.JText._('JM_REQUIRED_CONFIRM_PASSWORD')).show();
    		JM("#jm-input-confirm-pass").focus();
    		return false;
    	}
    	if(JM("#jm-input-pass").val()!=JM("#jm-input-confirm-pass").val()){
    		JM("#jm-registration-error").html(Joomla.JText._('JM_PASSWORD_NOT_MATCH')).show();
    		JM("#jm-input-confirm-pass").focus().select();
    		//JM("#jm-registration-error").show();
    		return false;
    	}
    	if(JM("#jm-input-email").val()==""){
    		JM("#jm-registration-error").html(Joomla.JText._('JM_REQUIRED_EMAIL')).show();
    		JM("#jm-input-email").focus();
    		return false;
    	}
    	var emailRegExp = /^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.([a-zA-Z]){2,4})$/;
    	if(!emailRegExp.test(JM("#jm-input-email").val())){		
    		JM("#jm-registration-error").html(Joomla.JText._('JM_EMAIL_INVALID')).show();
    		JM("#jm-input-email").focus().select();
            //JM("#jm-registration-error").show();
    		return false;
    	}
    	if(JM("#jm-input-confirm-email").val()==""){
    		JM("#jm-registration-error").html(Joomla.JText._('JM_REQUIRED_CONFIRM_EMAIL')).show();
    		JM("#jm-input-confirm-email").focus().select();
    		return false;
    	}
    	if(JM("#jm-input-email").val()!=JM("#jm-input-confirm-email").val()){
    		JM("#jm-registration-error").html(Joomla.JText._('JM_EMAIL_NOT_MATCH')).show();;
    		JM("#jm-input-confirm-email").focus().select();
    		return false;
    	}
		if(JM('#jm-captcha').length && JM('#jm-captcha').val()==''){
			JM("#jm-registration-error").html(Joomla.JText._('JM_CAPTCHA_REQUIRED')).show();
			JM('#jm-captcha').focus();
			return false;
		}	
